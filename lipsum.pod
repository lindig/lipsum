...
=item B<expand> [B<-f> I<fmt>] [I<file.lp>]

Extract each root code chunk to a file of the same name using format
I<fmt>. This is a quick way extract all code chunks in one go into
individual files.

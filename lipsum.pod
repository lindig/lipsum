...
references to the original source code location in I<file.lp>. For a list of
available formats, use B<tangle> with no arguments.

=item B<tangle> 

Emit list of available source code position formats. The best knwon format 
is C<cpp> as it is used by the C pre processor cpp(1).

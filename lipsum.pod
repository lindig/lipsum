...
# translated into manual format using pod2perl which comes with every Perl
...
B<lipsum> B<undefined> [I<file.lp>]
...
B<Lipsum> is a tool for literate programming in the tradition of NoWeb
...
markup. B<lipsum>'s primary task is to extract the source from a
literate program to make it available for compilation and execution.
This is traditionally called I<tangling>. B<Lipsum> is language agnostic
...
B<roots>), which is followed by options, parameters, and file names.
...
to I<stdout>. All commands accept an option B<--help> to show online
help.
...
I<file.lp>. For a list of available formats, use B<formats>.

...
source code which are seperated by a lightweigt markup. B<lipsum>'s primary
...
B<roots> list the available chunks. The extracted code optionally includes
references to  original source code locations in I<file.lp>. For a list of
available formats, use B<tangle -f>.
...
Emit I<file.lp>  with code chunks indented by 4 spaces. This makes the output 
suitable for Markdown provided the documentation is already in Markdown syntax.
...
See command B<copyright> for how to display the copyright notice and license.
